...
if (isset($_GET["date"])) {
    $search_date = $_GET["date"]; // e.g., "2024-04-18"

    $booking_rs = Database::search("SELECT reservation.id AS reservation_id, reservation.table AS table_number, reservation.date_time, reservation.message, reservation.email_sent_status_id, user.fname, user.lname, user.mobile FROM reservation INNER JOIN user ON reservation.user_id = user.id WHERE DATE(reservation.date_time) = '" . $search_date . "' ORDER BY reservation.date_time DESC");
...
?>
    ...
    ?>
        ...
        ?>
        <tr>
            <td colspan="8" class="text-center fw-bold text-danger">No bookings found for "<?php echo htmlspecialchars($search_date); ?>"</td>
        </tr>
<?php
    }
}
